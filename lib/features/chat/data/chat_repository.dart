...
abstract class ChatRepository {
  ...
  Stream<List<MessageModel>> watchMessages({required String channelName, int limit});
  Future<List<MessageModel>> fetchMessagesPage({
    required String channelName,
    int? endAtTimestamp,
    int limit,
  });
  Stream<String?> watchTyping({required String channelName, required String slotId});
  Stream<SlotModel?> watchSlot({required String channelName, required String slotId});
  Stream<bool?> watchSlotOnline({required String channelName, required String slotId});
...
class FirebaseChatRepository implements ChatRepository {
  ...
  @override
  Stream<List<MessageModel>> watchMessages({required String channelName, int limit = 30}) {
    final ref = _channelReference(channelName);
    if (ref == null) {
      return Stream<List<MessageModel>>.value(const <MessageModel>[]);
    }

    return ref.child('messages').orderByChild('timestamp').limitToLast(limit).onValue.map((event) {
      final value = event.snapshot.value;
      if (value is! Map) {
  ...
  @override
  Future<void> heartbeat({required String channelName, required String slotId}) async {
    final ref = _channelReference(channelName);
    if (ref == null) {
      return;
    }
    await ref.child('slots/$slotId/lastSeen').set(DateTime.now().millisecondsSinceEpoch);
  }
